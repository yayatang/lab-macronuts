...
all_samp <- read.csv(here::here('results/all_phases_clean_switched.csv'), stringsAsFactors=TRUE)

# === merge IRGA data with dry soil data ===
data_orig <- arrange(all_samp, sampleID, exp_count) # ordering the data in the table
...
# ggplot(ref_data, aes(x=exp_count, y=MC_ref_avg, color=factor(MC))) +
...
diff_summary_C <- merge(data_summary, ctrl_data, by=c('MC', 'exp_count')) %>%
...
irga_days <- data.frame(exp_count = unique(diff_summary_C$exp_count)) %>% arrange(exp_count)
...
max_days <- max(diff_summary_C$exp_count)
days_all <- seq(1, max_days)

# reduce number of variables carried, no need for diff_summary_C anymore
basic_data_C <- select(diff_summary_C, trt_ID, exp_count, diff_perday_C, se_reps)
...
colnames(grid_vals) <- c('trt_ID', 'exp_count')

# outer join blank table with CO2 data
gapped_data_C <- merge(grid_vals, basic_data_C, all.x=TRUE, by=c('trt_ID', 'exp_count'))
...
cumul_summary_C <- filled_full_C %>%
   group_by(trt_ID) %>%
   mutate(infer_cumul_C = order_by(exp_count, cumsum(infer_perday_C)))# %>%
# rename(mean_reps = diff_perday_C)

# # ===use this to check values are OK
# temptube <- cumul_summary_C %>% filter(trt_ID=='BGR')
# ggplot(temptube, aes(x=exp_count, y=infer_perday_C)) +
...
errorbar_cumul_C <- merge(irga_days, gapped_full_C, all.x=TRUE)# %>% arrange(exp_count, treatment)
...
for (i in 1:length(MC_vec)){
   ...
   print(
      ggplot(sub_data, aes(exp_count, plot_vals, color=factor(treatment), group=factor(treatment), ymin=u_ymin, ymax=u_ymax)) +
...
for (i in 1:length(trt_vec)){
   ...
   print(
      ggplot(data=sub_data, aes(x=exp_count, y=plot_vals, color=factor(MC), group=factor(MC), ymin=u_ymin, ymax=u_ymax)) +
