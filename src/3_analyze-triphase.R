...
data_with_C <- merge(data_orig, ctrl_data, by = c("MC", "exp_count"))
...
extra_ID <- unique(all_clean[c('sampleID', 'exp_count','phase', 'phase_count','MC','treatment', 'rep')])
phase_lookup <- unique(all_clean[c('exp_count', 'phase')])
...
# Create blank grid and interpolate [gapped_full]------------------------------------
...
daily_summ <- gapped_full %>%
   ...
   summarise_each(funs(mean(., na.rm=TRUE), se), diff_fromC_perday) %>%
...
cumul_phase_summ <- cumul_phase_indiv %>%
   group_by(MC, treatment, exp_count) %>%
   summarise_each(funs(mean(., na.rm=TRUE), se), phase_infer_cumul) %>%
...
cumul_all_summ <- cumul_all_indiv %>%
   group_by(MC, treatment, exp_count) %>%
   summarise_each(funs(mean(., na.rm=TRUE), se), infer_cumul) %>%
   rename(mean_reps = mean, se_reps = se) %>%
   merge(phase_lookup, by='exp_count')
