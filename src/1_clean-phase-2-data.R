...
get_info <- function(fileloc) {
    # fileloc <- 'IRGA 2-13.csv' # for DEBUGGING
    ...
    meta$incub_count_check <-
        round(difftime(day2, day1, units = "days")+1) # no +1 bc already included in start date
    # meta # for DEBUG
    ...
    if (meta$incub_count == 1) {
        day1_idx_flush <-
            (as.POSIXlt(samp$time_flush, format = '%H:%M:%S')$hour > 16 &
                 as.POSIXlt(samp$time_flush, format = '%H:%M:%S')$hour < 22)
        samp$day_flush[day1_idx_flush == T] <-
            as.character(as.Date(meta$day_flush) + 1)
    }
...
data_p2 <- all_samp %>%
    ...
    mutate(phase = 2, 
           exp_count = phase_count + 42) %>% 
    arrange(sampleID, exp_count)

write.csv(data_p2, file = here::here('results/all_clean_p2_switched.csv'), row.names=FALSE)