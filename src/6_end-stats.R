...
imported_data <- read_csv(paste0(here::here('results/tubes_to_plot_'),switch_file,'.csv')) %>% 
    filter(treatment != 'R')
...
for (k in seq_along(end_p)){
    ...
    ggsave(paste(paste0(here::here('results/',switch_file)),'phase', k, 'boxplot_cumul_dif.png', sep="_"), width=10, height=8, dpi=600)
    
    # =====summary stats=====
    
    # C mineralization GROSS means by treatment
    samp1_trt_gr <- group_by(end_data, treatment)
    summarise(samp1_trt_gr, 
              count = n(),
    ...
    #=== post hoc
    # TukeyHSD(res.diff.aov1, which="treatment")
